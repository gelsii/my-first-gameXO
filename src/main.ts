...
const displayHTML = async () => {
    try {
        const table = await fetch("my-first-game/src/pages/table.html");
        const displayTable = await table.text()

        const notification = await fetch("my-first-game/src/notifications/winnerPop.html");
        const displayNotification = await notification.text();

        const continuesGameNotif = await fetch("my-first-game/src/notifications/continuePop.html");
        const displayContinuesGameNotif = await continuesGameNotif.text();

        const tools = await fetch("my-first-game/src/tools/tools.html");
